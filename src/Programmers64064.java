...
public class Programmers64064 {
	...
	public static void dfs(String[] banned_id, String[] user_id, int idx, int n, boolean[] visited) {
		...
		if(n== banned_id.length) {
			...
			//여러 리스트 중 하나를 뽑아서 그 리스트 안에 있는 원소들과 방금 뽑은 원소들을 담은 temp 리스트의 원소와 비교
		...
		for(int i=0; i<user_id.length; i++) {
			...
			//정규표현식에 매칭되고, 길이가 같은 경우
			if(matcher.find() && user_id[i].length() == banned_id[idx].length()) {
				//제제 아이디로 체크하지 않은 경우 check
				if(!visited[i]) {
					visited[i] = true;
					dfs(banned_id, user_id, idx+1, n+1, visited);	//다음 banned_id와 매쳉되는 제재 아이디 찾기
					visited[i] = false;
				}
			}
